"""
libRaptorQ Python API:
libRaptorQ is a C++11 implementation of the RaptorQ Forward Error Correction, as described in the RFC6330.
Since the RFC is needlessly complex, a simpler and slightly faster, low level API is available, too.
libRaptorQ can be used as a C++11 header-only library, or compiled and linked with C and C++98 bindings.
"""
...
class RQEncoderBlock(object):
    ...
    def encode_iter(self, repair_rate=None, repair_count_max=None):
        ...
        for esi in range(n + nr): yield self.encode(esi)

    def __iter__(self):
        return self.encode_iter()
